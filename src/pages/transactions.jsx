...
export default function Transactions() {
  ...
  const getMonthOptions = () => {
    ...
    const nextJanLabel = nextYearJan.toLocaleDateString('en-US', { year: 'numeric', month: 'long' });
    const nextJanShortLabel = nextYearJan.toLocaleDateString('en-US', { month: 'short', year: '2-digit' });
    options.push({ 
      value: nextJanValue, 
      label: nextJanLabel, 
  ...
  const removeDuplicateTransactions = (transactions) => {
    ...
    for (const transaction of transactions) {
      ...
      if (!seen.has(key)) {
        ...
        console.warn(`Duplicate transaction detected and filtered out:`, transaction);
      }
    }
  ...
  const fetchTransactions = async () => {
    ...
    if (data.length !== uniqueData.length) {
      console.log(`Filtered out ${data.length - uniqueData.length} duplicate transactions`);
    }
  ...
  const generateRecurringTransactions = (transaction) => {
    ...
    const recurringSeriesId = `${transaction.description}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const handleAdd = async () => {
    ...
    try {
      if (newTransaction.recurring !== 'none') {
        ...
        const recurringTransactions = generateRecurringTransactions(newTransaction);
        console.log(`Adding ${recurringTransactions.length} recurring transactions`);
  ...
  const handleDelete = (transactionId) => {
    ...
    if (transaction && transaction.isRecurring && transaction.recurringSeriesId) {
  ...
  const handleDeleteConfirmed = async (transactionId, choice) => {
    ...
    if (choice === 'all-series' && transaction?.recurringSeriesId) {
      ...
      console.log(`Deleting entire recurring series: ${seriesTransactions.length} transactions`);
  ...
  const handleSave = async () => {
    ...
    await updateTransaction(user.uid, editingTransactionId, {
      ...
      date: editingTransaction.date,
        ...
        const seriesTransactions = transactions.filter(t => 
          t.recurringSeriesId === editingTransaction.recurringSeriesId
        );
        ...
        if (originalFrequency !== newFrequency) {
          console.log(`Frequency change detected: ${originalFrequency} → ${newFrequency}`);
          console.log(`Deleting ${seriesTransactions.length} old transactions and creating new series`);
          ...
          const recurringTransactions = generateRecurringTransactions(editingTransaction);
          console.log(`Generated ${recurringTransactions.length} new transactions with ${newFrequency} frequency`);
          ...
          const originalEndDateObj = originalEndDate ? new Date(originalEndDate + 'T23:59:59') : null;
          const newEndDateObj = newEndDate ? new Date(newEndDate + 'T23:59:59') : null;
          
          // Check if end date was extended (need to generate more transactions)
          if ((!originalEndDateObj && newEndDateObj) || (originalEndDateObj && newEndDateObj && newEndDateObj > originalEndDateObj)) {
            console.log(`End date extended: ${originalEndDate || 'none'} → ${newEndDate}`);
            ...
            const newRecurringTransactions = generateRecurringTransactions(editingTransaction);
            console.log(`Generated ${newRecurringTransactions.length} transactions with extended end date`);
            ...
            const transactionsToDelete = seriesTransactions.filter(transaction => {
            ...
            console.log(`End date shortened: ${originalEndDate} → ${newEndDate}`);
            console.log(`Deleting ${transactionsToDelete.length} transactions after new end date`);
            ...
            const transactionsToKeep = seriesTransactions.filter(transaction => {
              const transactionMonth = transaction.date.substring(0, 7);
              if (newEndDate) {
                const newEndMonth = newEndDate.substring(0, 7);
                return transactionMonth <= newEndMonth;
                        ...
                        <td colSpan="4" className="edit-full-row">
                          ...
                          >
                            ...
                            <input
                                    ...
                                    onChange={(e) => setEditingTransaction({...editingTransaction, recurringEndDate: e.target.value})}
                              className="edit-input-sm"
                            />
                          </div>
                              )}
                          ...
                          <div className="edit-actions">
                                  <button onClick={handleSave} className="save-btn-sm">✓ Save</button>
                                  <button onClick={handleCancel} className="cancel-btn-sm">✕ Cancel</button>
                                </div>
                              </div>
                            </div>
                            ...
                            {editingTransaction.recurring !== 'none' && (
                              <div className="edit-note">
                                <small>Note: Editing a recurring transaction will replace the original with a new recurring series.</small>
                              </div>
                            )}
                            
                            {editingTransaction.recurringSeriesId && editChoice === 'all-series' && 
                             editingTransaction.recurring !== 'none' && 
                                                          (() => {
                               const originalTransaction = transactions.find(t => t.id === editingTransactionId);
                               const originalFrequency = originalTransaction?.recurring;
                               const originalEndDate = originalTransaction?.recurringEndDate || "";
                               const newFrequency = editingTransaction.recurring;
                               const newEndDate = editingTransaction.recurringEndDate || "";
                               return originalFrequency && (originalFrequency !== newFrequency || originalEndDate !== newEndDate);
                             })() && (
                              <div className="frequency-change-warning">
                                <strong>⚠️ Recurring Settings Change Detected:</strong>
                                <br />
                                <small>
                                   {(() => {
                                     const originalTransaction = transactions.find(t => t.id === editingTransactionId);
                                     ...
                                     const endDateChanged = originalEndDate !== newEndDate;
                                     
                                     if (frequencyChanged && endDateChanged) {
                                       return <>Changing frequency from <strong>{originalFrequency}</strong> to <strong>{newFrequency}</strong> and end date from <strong>{originalEndDate || 'none'}</strong> to <strong>{newEndDate || 'none'}</strong> will recreate the entire series</>;
                                     } else if (frequencyChanged) {
                                       return <>Changing frequency from <strong>{originalFrequency}</strong> to <strong>{newFrequency}</strong> will recreate the entire series</>;
                                     } else if (endDateChanged) {
                                       const originalEndDateObj = originalEndDate ? new Date(originalEndDate + 'T23:59:59') : null;
                                       const newEndDateObj = newEndDate ? new Date(newEndDate + 'T23:59:59') : null;
                                       
                                       if ((!originalEndDateObj && newEndDateObj) || (originalEndDateObj && newEndDateObj && newEndDateObj > originalEndDateObj)) {
                                         return <>Changing end date from <strong>{originalEndDate || 'none'}</strong> to <strong>{newEndDate}</strong> will recreate the series with the extended end date</>;
                                       } else {
                                         return <>Changing end date from <strong>{originalEndDate}</strong> to <strong>{newEndDate || 'none'}</strong> will delete transactions after the new end date</>;
                                       }
                                     }
                                   })()}
                                  <br />
                                  {(() => {
                                    ...
                                    if (frequencyChanged) {
                                      return (
                                        <>
                                          • Delete all existing transactions in this series<br />
                                          • Create new transactions with {newFrequency} frequency<br />
                                          • Generate proper spacing based on the new frequency
                                        </>
                                      );
                                    } else if (endDateChanged) {
                                      const originalEndDateObj = originalEndDate ? new Date(originalEndDate + 'T23:59:59') : null;
                                      ...
                                      if ((!originalEndDateObj && newEndDateObj) || (originalEndDateObj && newEndDateObj && newEndDateObj > originalEndDateObj)) {
                                        return (
                                          <>
                                            • Delete existing series and recreate with extended end date<br />
                                            • Include all transactions from start to new end date
                                          </>
                                        );
                                      } else {
                                        return (
                                          <>
                                            • Delete transactions after new end date<br />
                                            • Update remaining transactions with new details
                                          </>
                                        );
                                      }
                                    } else {
                                      return (
                                        <>
                                          • Update all transactions with new details<br />
                                          • Keep existing dates and frequency
                                        </>
                                      );
                                    }
                                  })()}
                      ...
                      <>
                        ...
                        <td className="description-cell">
                          <div className="description-main">
                            {transaction.description}
                            {transaction.isRecurring && (
                              <span className="recurring-badge" title="Recurring Transaction">🔄</span>
                            )}
                          </div>
                          {transaction.category && (
                        ...
                        <td className="actions-cell">
                          <button onClick={() => handleEdit(transaction)} className="edit-btn-sm" title="Edit">✏️</button>
                          <button onClick={() => handleDelete(transaction.id)} className="delete-btn-sm" title="Delete">🗑️</button>
                        </td>
                      </>
                    )}
                  </tr>
          ...
          <div className='modal-content'>
            ...
            <div className='modal-body'>
              ...
              <div className="recurring-section">
                <label className="recurring-label">Recurring Transaction:</label>
                <select
                ...
                {newTransaction.recurring !== 'none' && (
                  <>
                    ...
                    <small className="recurring-note">
                      If no end date is provided, transactions will be created for 1 year.
                    </small>
                  </>
                )}
              </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowRecurringEditModal(false)}>×</button>
            </div>
            <div className="modal-body">
              <p className="recurring-choice-text">
                This is part of a recurring transaction series. What would you like to edit?
              </p>
              <div className="recurring-choice-buttons">
                ...
                >
                  ...
                  <div className="choice-content">
                    ...
                    <div className="choice-description">
                      Change only this transaction. It will be removed from the recurring series.
                    </div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="choice-content">
                    ...
                    <div className="choice-description">
                      Apply changes to all transactions in this recurring series.
                    </div>
                  </div>
                </button>
              </div>
            ...
            <div className="modal-footer">
              <button onClick={() => setShowRecurringEditModal(false)} className="modal-cancel-btn">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowRecurringDeleteModal(false)}>×</button>
            </div>
            <div className="modal-body">
              <p className="recurring-choice-text">
                This is part of a recurring transaction series. What would you like to delete?
              </p>
              <div className="recurring-choice-buttons">
                <button 
                ...
                >
                  ...
                  <div className="choice-content">
                    ...
                    <div className="choice-description">
                      Remove only this transaction. Other transactions in the series will remain.
                    </div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="choice-content">
                    ...
                    <div className="choice-description">
                      Remove all {(() => {
                        const transaction = transactions.find(t => t.id === deletingTransactionId);
                        const count = transactions.filter(t => t.recurringSeriesId === transaction?.recurringSeriesId).length;
                        return count;
            ...
            <div className="modal-footer">
              <button onClick={() => setShowRecurringDeleteModal(false)} className="modal-cancel-btn">
                Cancel
              </button>
            </div>
          </div>
